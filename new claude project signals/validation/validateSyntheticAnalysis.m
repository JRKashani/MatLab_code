...
%   issues that break the validation itself still throw MATLAB errors.
    ...
    if isfield(data, 'Config') && isfield(data.Config, 'SineComponents')
        for k = 1:numel(data.Config.SineComponents)
            ...
            % Inspect this component's frequency neighborhood; the global
            % maximum belongs only to the strongest tone in a multitone file.
            candidates = find(f > 0 & abs(f - s.freq) <= max(tolerance.freqHz, data.Fs / numel(seg)));
            [peakAmp, localIdx] = max(spec(candidates));
            estFreq = f(candidates(localIdx));
    ...
    if isfield(data, 'pureSineSignal') && isfield(data, 'Fs')
        ...
        if isempty(data.Config) || ~isfield(data.Config, 'SineComponents') || isempty(data.Config.SineComponents)
            validation = validateAddFailure(validation, 'FFT AMPLITUDE', 'No configured sine to validate FFT amplitude.');
        else
